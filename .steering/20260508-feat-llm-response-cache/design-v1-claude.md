...
ストアは Python 標準ライブラリの `sqlite3` を採用。理由:
- 単一ファイル / atomic writes / 並列読み書き安全 (Best-of-N 並列化が将来戻ってきても OK)
- 既存依存ゼロ（`shelve` は並列書き込みに弱く、`diskcache` は新規依存）
- スキーマも単純（key TEXT PRIMARY KEY, value BLOB, created_at INTEGER, hit_count INTEGER）
...
```
{
  ...
  "system_prompt": str,
  "user_prompt": str,
  "temperature": float,           # 含める（temp 違いは別エントリ）
  "max_tokens": int | null,       # text のみ。structured は固定 4096 → null
  "response_schema_hash": str | null  # structured のみ。Pydantic JSON Schema の SHA-256
}
```

含める判断:
- **temperature を含める**: 現実の Phase 1 (固定 0.7) / Phase 3 (固定) では HIT、Phase 2 (retry で変動) では MISS。
  これは findings.md の「Phase 1+3 が cache の主戦場」と整合。temperature を除外すると Phase 2 で
  確率変動を失わせ本番品質を毀損するリスク
- **response_schema_hash を含める**: Pydantic モデル変更で旧キャッシュが誤適用されないように
- **system_prompt / user_prompt は raw text**: ハッシュ前の原文を canonical JSON に乗せる。これによりプロンプト 1 文字でも違えば別キー
...
| **temperature をキーに含める** | **採用** | Phase 1+3 の HIT を取りつつ Phase 2 の確率変動を保つ |
