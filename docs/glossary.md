...
過去の使用状況に基づく絶対禁止事項を Generator プロンプトの**冒頭**に配置する仕組み。余韻パターン・場面構造・書き出しパターンの上限到達や前日同一構造を「違反した日記は無条件で却下される」と宣言し、LLM の注意バイアス（プロンプト先頭の指示に強く従う傾向）を活用する。
...
Phase 2 生成直後に実行される軽量なルールベースチェック。余韻パターン上限・場面構造の連続使用/上限・主題語 per-day 上限・書き出しパターン上限の5項目を検査し、違反メッセージのリストを返す。
Critic（Phase 3）が検査しない構造的多様性を補完する。場面構造違反がある場合は Critic Pass 時でも1回だけ再試行を強制する。
...
整然とした文体の場合は `persona_deviation` を減点する。
...
本シナリオでは Day 4（`emotional_impact: -0.9`）が該当する。
