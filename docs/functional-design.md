...
| **例外フロー** | 前日の状態ログが存在しない場合 → エラーメッセージを出力し、`--from-scratch` オプションの使用を提案する |
...
| F-06-06 | 既存の出力ファイルが存在する場合、上書きされること（`--no-overwrite` オプションで抑制可能） |
...
| `--config` | `str` | (なし) | 設定ファイル（.env）のパス。未指定時は環境変数を使用 |
| `--no-overwrite` | `flag` | `False` | 既存の出力ファイルが存在する場合、上書きしない |
| `--from-scratch` | `flag` | `False` | 前日の状態ログを無視し、`h_0` から開始する |
...
Verbose モード（`--verbose`）:
```
[CSDG] Config: model=gpt-4o, max_retries=3, initial_temp=0.7
...
| `CSDG_LLM_MODEL` | 任意 | `gpt-4o` | 使用するLLMモデル名 |
| `CSDG_LLM_BASE_URL` | 任意 | `https://api.openai.com/v1` | LLM APIのベースURL |
...
| 指標 | 目標値 | 備考 |
|---|---|---|
| 全Day生成成功率 | 100%（フォールバック含む） | Dayスキップ = 0 を目指す |
| 平均 CriticScore 合計 | 12.0 以上（3スコア合計15点満点中） | |
| フォールバック発生率 | 10% 以下（7Day中0〜1回） | |
| 平均リトライ回数 | 1.0 以下 | |
| Day 4 の Pass率 | 80% 以上（リトライ含む） | 最もシビアなDay |
