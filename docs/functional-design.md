...
| F-01-08 | `human_condition`（HumanCondition サブモデル）が `CharacterState` に含まれ、`sleep_quality`, `physical_energy`, `mood_baseline`, `cognitive_load`, `emotional_conflict` が前日の状態から自動導出されること |
...
| F-02-05 | `human_condition`（特に `sleep_quality`, `cognitive_load`, `emotional_conflict`）がプロンプトに注入され、日記の文体・トーンに影響を与えること |
...
| F-03-07 | `CriticScore` に `hook_strength`（0.0〜1.0）が診断専用フィールドとして含まれ、Pass/Reject判定には使用されないこと |
...
| F-04-02 | リトライ時の Temperature は区分線形 [0.70, 0.60, 0.45, 0.30] で段階的に減衰すること |
| F-04-03 | 最大リトライ回数は `max_retries` (デフォルト3) 回であること。初回を含めて最大4回の試行 (Temperature スケジュール: [0.70, 0.60, 0.45, 0.30]) |
| F-04-04 | 最大リトライ回数を超えた場合、全候補の中から CriticScore 合計値が最大のものを採用すること（Best-of-N） |
| F-04-05 | Critic Pass でも構造的制約違反がある場合、ボーナス再試行 (予算外1回) が実行されること |
| F-04-06 | ボーナス再試行時、`structural_violations` が Actor の `critical_constraints` と `revision_instruction` の両方に二重注入されること |
| F-04-07 | フック関連違反のボーナス再試行では、`critical_constraints` のフック制約が具体的修正例付きの強化版に置換されること |
| F-04-08 | 構造的制約バリデーションは11項目 (余韻パターン上限、場面構造連続/上限、主題語上限、書き出しパターン上限、冒頭/余韻テキスト重複、禁止余韻パターン、本文フレーズ重複、文字数範囲、末尾フック弱さ) を検査すること |
...
| ID | 要件 |
|---|---|
| F-08-01 | API過負荷エラー (OverloadedError) 発生時、指数バックオフ（ベース30秒 × 2^n、最大3回リトライ）で再試行すること。Anthropic クライアントは内蔵リトライ (max_retries=5) も適用される |
...
プロンプトチューニングサイクル（同一プロンプト・同一パラメータで複数回 LLM を呼び出す状況）の
回転率を上げるため、LLM クライアント前段にディスクベースの KV キャッシュを設ける。

| ID | 要件 |
|---|---|
| F-09-01 | キャッシュキーは `(provider, model, system_prompt, user_prompt, temperature, max_tokens, response_schema_hash)` の正規化 SHA-256 とすること。同一入力で必ず同一キーを返す |
| F-09-02 | キャッシュストアは SQLite で実装し、既定パスは `~/.cache/csdg/llm/cache.sqlite` とすること（XDG 準拠 / プロジェクト外） |
| F-09-03 | `--no-cache` CLI フラグでキャッシュを bypass できること（A/B 比較・本番再現性確認に使用） |
| F-09-04 | キャッシュ HIT/MISS は `logger.info` に `kind` (structured/text) / `model` / `response_model` 名を含めて出力すること。Phase 情報の付与は wrapper では取得できないため V2 に先送り（生成時間ゼロ ≒ Phase duration が極小値で `throughput_report.py` が効果を捉える） |
| F-09-05 | キャッシュ書き込みは LLM 呼び出し成功時のみ。例外発生時は記録せず通常のリトライ・フォールバック経路に従うこと |
| F-09-06 | キャッシュ自動失効 (TTL) は実装しない。手動削除手順を docs に明記する (`rm -rf ~/.cache/csdg/llm/`) |
| F-09-07 | 既存の `LLMClient` 抽象インターフェース（`generate_structured`, `generate_text`）を破壊的変更しないこと。Decorator パターンで透過的にラップする |
| F-09-08 | キャッシュ HIT 時に JSON 復号 / Pydantic 検証で破損を検出した場合は該当 row を evict し、inner LLMClient を MISS として再呼び出しすること (Self-Healing) |
| F-09-09 | `gemini_fallback_models` が設定されている場合は、実際に応答したモデルが key に反映できないためキャッシュを自動無効化すること (warning ログ出力) |
| F-09-10 | DB ファイルは owner のみ read/write 可能 (mode 0600) で作成すること |
...
| `--no-cache` | `flag` | `False` | LLM レスポンスキャッシュを bypass する。F-09 参照 |
...
[Day 4] Phase 2: Content Generation (retry, temp=0.60) ... OK (3.8s)
