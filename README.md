...
とこみが運営する個人ブログ「余白のログ」の記事として、1 日あたり約 400 文字のブログ日記を出力する。
...
- **前日接続 (prev_day_ending)** -- Day 2以降で前日の末尾段落を Generator / Critic 両プロンプトに注入し、1文目で前日の出来事・感情・人物に具体的に言及させる。テーマレベルではなく出来事レベルの接続を実現
- **未解決フック** -- Day 1〜6 の末尾 2 文以内に「未解決の種」(未回収の出来事、予告的行動、解決されていない違和感、伏線的具体物等) の埋め込みを指示。余韻（文章を閉じる）とフック（文章を開く）の区別を明示。1 エントリにつきフック 1 つ限定（散漫化防止）
- **フック回収義務** -- Day 2以降で前日のフックに最低 1 つ触れることを義務化。Day 1〜5 ではフックを完全に閉じず変形させる連続性ルールを適用
- **hook_strength 診断** -- Critic が 0.0〜1.0 でフック強度を評価し generation_log.json に記録 (Pass/Reject 判定には不使用)。前日フック未回収時は 0.4 以下に制約
- **出力前セルフチェック** -- Generator に前日接続・フック存在・余韻混同・感情結論の 4 項目セルフチェックを指示
...
8. 禁止余韻パターン (「本当に両立」「の間にある溝」「本当に分離できる」)
9. 本文フレーズの Day 間重複 (15 文字以上の完全一致)
10. 文字数の理想範囲 (450 文字以下)
11. 末尾フックの弱さ (Day 1-6: 弱い修辞疑問・感情の結論で閉じるフックを検出)

Critic Pass + 構造的違反時は**ボーナス再試行** (リトライ予算を消費しない 1 回限定の追加試行) を実行。フック関連違反では `structural_violations` を Actor に二重注入 (critical_constraints 強化版 + revision_instruction ガイダンス) し、具体的な修正例付きでリトライの実効性を高める。Best-of-N フォールバック時は構造的違反数をペナルティとしてスコアから減算し、違反のない候補を優先選択。
...
| Layer 3 | `LLMJudge` | 定性的 | 0.25 | LLM による temporal / emotional / persona 評価 (L1/L2 結果を参照基準として構造化注入) + `hook_strength` 診断値 (0.0〜1.0) |

最終スコアは純粋な加重平均の `round()` で決定 (base score 2.5)。Veto 対象の致命的違反 (禁止一人称 / 文字数極端逸脱 / 高 trigram 重複) は安全制限をバイパスして強制的にスコア上限を適用。`hook_strength` は診断専用フィールド（Pass/Reject 判定には不使用）として `generation_log.json` に記録される。
