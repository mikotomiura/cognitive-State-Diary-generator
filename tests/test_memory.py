...
class TestLLMExtraction:
    """_llm_extract_beliefs_and_themes の LLM 抽出テスト."""

    @pytest.mark.asyncio()
    async def test_prompt_not_found_continues_gracefully(self, tmp_path: Path) -> None:
        """Prompt_MemoryExtract.md が存在しない場合、エラーにならない."""
    ...
    @pytest.mark.asyncio()
    async def test_llm_extraction_adds_beliefs_and_themes(self, tmp_path: Path) -> None:
        """LLM が MemoryExtraction を返した場合、beliefs/themes が追加される."""
        # プロンプトファイルを作成
        ...
        mock_client.generate_structured.return_value = MemoryExtraction(
            new_beliefs=["効率は善ではない", "静寂の中に真実がある"],
            new_themes=["都市と自然の対比"],
        )
        ...
        assert "静寂の中に真実がある" in manager.memory.long_term.beliefs
        assert "都市と自然の対比" in manager.memory.long_term.recurring_themes

    @pytest.mark.asyncio()
    async def test_llm_extraction_no_duplicates(self, tmp_path: Path) -> None:
        """重複する belief は追加されない."""
        prompt = tmp_path / "Prompt_MemoryExtract.md"
        prompt.write_text(
            "Extract:\n{evicted_entries}\n{current_beliefs}\n{current_themes}",
        ...
        memory = Memory(
            short_term=ShortTermMemory(window_size=3),
            long_term=LongTermMemory(beliefs=["既存の信念"]),
        )
        manager = MemoryManager(window_size=3, memory=memory, prompts_dir=tmp_path)
        mock_client = AsyncMock()
        mock_client.generate_structured.return_value = MemoryExtraction(
            new_beliefs=["既存の信念", "新しい信念"],
            new_themes=[],
        )
    ...
    async def test_llm_extraction_failure_continues(self, tmp_path: Path) -> None:
        ...
        # beliefs は変化しない
        assert len(manager.memory.long_term.beliefs) == 0
