...
class TestTemperatureScheduleProperty:
    ...
    def test_piecewise_linear_schedule(self, config: CSDGConfig) -> None:
        """区分線形スケジュールの固定値を返す。"""
        schedule = config.temperature_schedule
        assert schedule == [0.70, 0.60, 0.45, 0.30]

    def test_schedule_monotonically_decreasing(self, config: CSDGConfig) -> None:
        """スケジュールが単調減少である。"""
        schedule = config.temperature_schedule
    ...
    def test_schedule_length(self, config: CSDGConfig) -> None:
        """スケジュールの長さが4（固定）。"""
        assert len(config.temperature_schedule) == 4

    def test_schedule_boundaries(self, config: CSDGConfig) -> None:
        """初回は0.70、最終は0.30。"""
        schedule = config.temperature_schedule
        assert schedule[0] == pytest.approx(0.70)
        assert schedule[-1] == pytest.approx(0.30)
...
class TestCriticWeightsProperty:
    ...
    def test_returns_critic_weights_instance(self, config: CSDGConfig) -> None:
        ...
        assert weights.rule_based == 0.40
        assert weights.statistical == 0.35
        assert weights.llm_judge == 0.25
